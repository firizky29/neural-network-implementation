        ...
        for i in range(len(self.layers)):
            ...
            for j in range(self.layers[i].num_of_neurons):
                ...
                for k in range(self.layers[i].num_of_input):
                    G.add_edge(get_id(i,k+1), get_id(i+1,j+1), weight=layer.w[j][k])
                G.nodes[get_id(i+1,j+1)]["subset"] = i+1

        pos = nx.multipartite_layout(G)
        nx.draw(G, pos, with_labels=True, font_weight='bold', node_color=colors)
        nx.draw_networkx_edge_labels(G, pos, edge_labels=nx.get_edge_attributes(G, 'weight'), label_pos=0.8, font_size=8)
        plt.show()
